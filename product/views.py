...
from .models import ProductDetail, ProductCategory, AddToCard, city, PaymentMode, Order, Brand, Compare, Review, TrackDetail
...
# add product while orering any item with their given specification
def addproduct(request):
    ...
    define_count = AddToCard.objects.filter(user_key = request.user, product_key = product)
    the_divine_thing = define_count.aggregate(total_data=Sum('product_quantity'))
    add_cart_count = add_obj.filter(user_key = request.user, is_wishlist = False).count()

    data = {
        'add_cart_count':add_cart_count,
        'main_quantity':main_quantity,
        'product_all_count':the_divine_thing['total_data'],
    }
...
def product_detail(request, slug):
    ...
    add_obj = AddToCard.objects.filter(user_key = request.user)
    product_data = ProductDetail.objects.get(slug = slug)
    total = add_obj.filter(user_key = request.user,product_key = product_data).count()
    partial_count = add_obj.fimlter(user_key = request.user, product_key = product_data).aggregate(total_piece = Sum('product_quantity'))
    add_cart_count = add_obj.filter(user_key = request.user, is_wishlist = False).count()
    add_wishlist_count = add_obj.filter(user_key = request.user, is_wishlist = True).count()
    total_data_product = ProductDetail.objects.filter(slug = slug).aggregate(total_piece = Sum('product_stock'))
    review_product = Review.objects.filter(product_key = product_data)
...
def ConfirmOrder(request):
    if request.method == "POST":
        ...
        for pro in product:
            ...
            product_dd = ProductDetail.objects.get(pk = product_data.product_key.id)
            product_quantity = product_dd.product_stock

            print(product_dd)

            if int(quantity1)>int(product_quantity):
                print("you cant order"+ product_dd.product_name)
            else:
...
def tracking(request):
    if request.method == 'POST':
        ...
        order_obj = Order.objects.filter(track_key__in = order)
        print(order_obj)

        if order_obj:
            user_data = Order.objects.get(pk__in = order_obj)
            if request.user == user_data.user_key:
                odetails = Order.objects.filter(track_key__in = order)
                print(odetails)
                context = {
                    'odetails':odetails
                }
...
def track(request, track):
    ...
    track_data = TrackDetail.objects.get(number = track)
    order_data = Order.objects.get(track_key = track_data)

    if request.user == order_data.user_key:
        odetails = Order.objects.filter(track_key = track_data)
        print(odetails)
        context = {
            'odetails':odetails
        }
